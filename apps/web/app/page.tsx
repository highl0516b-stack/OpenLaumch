...
const channels = [
  "X / Twitter", "LinkedIn", "小紅書", "YouTube Shorts", "Discord",
  "Telegram", "Product Hunt", "Hacker News", "Indie Hackers", "Email",
] as const;
    ...
    <main className="page-shell">
      <section className="hero">
        ...
        <div className="hero-card">
          <div className="metric-row"><span>10 min</span><small>從想法到 launch command center</small></div>
          <div className="metric-row"><span>30 days</span><small>自動規劃持續跟進節奏</small></div>
          <div className="metric-row"><span>MCP</span><small>預留工具生態與 agent adapter</small></div>
        </div>
      </section>
      ...
      <Section id="channels" title="一鍵全網預留接口" subtitle="MVP 先產生文案包，後續接上真正分發與 CRM 工具。">
        <div className="channel-list">
          {channels.map((channel) => <span key={channel} className="channel-pill">{channel}</span>)}
        </div>
      </Section>
    </main>
  );
