...
const gateway = new McpGateway({
  ...
  allowedPaths,
  rateLimit: { maxCalls: RATE_LIMIT_MAX_CALLS, windowMs: RATE_LIMIT_WINDOW_MS },
});

const server = createServer(async (request, response) => {
  try { await route(request, response); } catch (error) { sendError(response, error); }
});
...
async function route(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (method === "OPTIONS") { sendJson(response, 204, {}); return; }
  ...
  if (url.pathname === "/api/mcp/tools/call" && method === "POST") {
    ...
    if (!name) { sendJson(response, 400, { ok: false, error: "Missing MCP tool name." }); return; }
  ...
  if (url.pathname === "/api/launch/generate" && method === "POST") {
    ...
    if (missing.length > 0) { sendJson(response, 400, { ok: false, error: "Missing required fields", missing }); return; }
    const plan = generateLaunchPlan(brief);
    sendJson(response, 200, { ok: true, plan, audit: { tenantId: context.tenantId, mode: context.mode, action: "launch.generate", durationMs: Date.now() - startedAt } });
    return;
  }
  ...
  if (url.pathname.startsWith("/api/webhooks/") && method === "POST") {
    const body = await readJson(request);
    sendJson(response, 202, { ok: true, event: url.pathname.replace("/api/webhooks/", ""), tenantId: requestContext(request).tenantId, receivedAt: new Date().toISOString(), bodyPreview: safePreview(body) });
    return;
  }
...
function healthPayload(request: IncomingMessage) {
  const context = requestContext(request);
  return { ok: true, service: "openlaunch-api", timestamp: new Date().toISOString(), uptimeSeconds: Math.round(process.uptime()), tenantId: context.tenantId, mode: context.mode, mcpTools: defaultToolDefinitions().length };
}
...
function normalizeLaunchBrief(body: unknown): LaunchBrief {
  if (!isRecord(body)) { return { productName: "", oneLiner: "", audience: "", problem: "", launchGoal: "waitlist", channels: [] }; }
  return {
...
function validateLaunchBrief(brief: LaunchBrief): string[] {
  ...
  return required.filter((field) => { const value = brief[field]; return Array.isArray(value) ? value.length === 0 : !value; });
}

function createLead(body: unknown): LeadRecord {
  const record = isRecord(body) ? body : {};
  const status = leadStatus(record.status) ?? "new";
  return { id: `lead_${Date.now().toString(36)}`, name: stringField(record, "name") ?? "Anonymous lead", email: stringField(record, "email") ?? "", source: stringField(record, "source") ?? "manual", score: numberField(record, "score") ?? 50, status, createdAt: new Date().toISOString() };
}
...
async function readJson(request: IncomingMessage, maxBytes = MAX_BODY_BYTES): Promise<unknown> {
  ...
  for await (const chunk of request) {
    ...
    if (totalBytes > maxBytes) { const error = new Error("Request body too large") as ApiError; error.statusCode = 413; throw error; }
    chunks.push(buffer);
  }
  if (totalBytes === 0) return {};
  try { return JSON.parse(Buffer.concat(chunks).toString("utf8")) as unknown; } catch { const error = new Error("Invalid JSON body") as ApiError; error.statusCode = 400; throw error; }
}

function checkRateLimit(key: string): boolean {
  const now = Date.now();
  const bucket = (rateLimitBuckets.get(key) ?? []).filter((timestamp) => now - timestamp < RATE_LIMIT_WINDOW_MS);
  if (bucket.length >= RATE_LIMIT_MAX_CALLS) { rateLimitBuckets.set(key, bucket); return false; }
  bucket.push(now);
...
function sendError(response: ServerResponse, error: unknown): void {
  ...
  sendJson(response, statusCode, { ok: false, error: { code: statusCode === 400 ? "bad_request" : statusCode === 404 ? "not_found" : statusCode === 413 ? "payload_too_large" : statusCode === 429 ? "rate_limited" : "internal_error", message: redactSecrets(message) } });
}

function safePreview(value: unknown, maxBytes = 500): unknown {
  const text = typeof value === "string" ? value : JSON.stringify(value);
  const safeText = redactSecrets(text).slice(0, maxBytes);
  try { return JSON.parse(safeText) as unknown; } catch { return safeText; }
}
...
function channelKeys(value: unknown): ChannelKey[] {
  const valid = new Set<ChannelKey>(["x","linkedin","xiaohongshu","youtube_shorts","discord","telegram","product_hunt","hacker_news","indie_hackers","email","bluesky","threads","reddit","tiktok","wechat","substack","medium","youtube","press"]);
...
function firstStringHeader(...headers: Array<string | string[] | undefined>): string | undefined {
  for (const header of headers) { if (typeof header === "string" && header.length > 0) return header; }
  return undefined;
}
